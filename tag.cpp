...
void Tagger::loadModel(char fname[]) {
...
double Tagger::getTransProb(int curState, int toTranState) {
    return log(double(transFreq[curState][toTranState] + 1) / (typeFreq[curState] + 49));
}

void Tagger::hmm(std::deque<WordToken*> ret) {
...
std::wstring Tagger::s2ws(const std::string& str) {
    if (str.empty()) {
        return L"";
    }
